# Code for evaluating tasks from the DecaNLP Benchmarks.
# The code is from this repo:
...
def compute_metric_keys(rouge=False, bleu=False, corpus_f1=False,
                        logical_form=False, args=None, dialogue=False):
    metric_keys = []
    if logical_form:
        metric_keys += ['lfem']
    if dialogue:
...
def compute_metrics(greedy, answer, rouge=False, bleu=False, corpus_f1=False, logical_form=False, args=None, dialogue=False):
    ...
    if logical_form:
        greedy = [g.lower() for g in greedy]
